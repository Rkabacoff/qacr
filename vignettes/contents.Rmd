...
## Missing Data
The iris dataset did not contain any missing data.  Therefore, in order to understand how the `contents` function treats missing data, we must use a differet dataset.  The **airquality** dataset, which records the daily air quality measurements in New York for May to September 1973, contains missing information for two variables, *Ozone* and *Solar.R*. The data is documented is detail in ?airquality
```{r}
summary(airquality)
```

If a user inputed a dataset with missing values into the `contents` function, the following tables would be produced:
```{r}
contents(airquality)
```

For more information on this function, please use `?contents`.
