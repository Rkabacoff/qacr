...
**`newdata <- recodes(data, variables, from values, to values)`**
...
We will use the dollar sign (**\$**) to represent age. The symbol (**\|**) means **or**.
...
This works because once the value for an observations meets a criteria (working left to right), it is recoded. It is not changed again by the same `recodes` statement.
